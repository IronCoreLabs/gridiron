...
fp31!(
    ...
    // 540164672828597150601552066704871144340865164390233716165828766698348036766646674962138582407269471436938515957052715279568068096452253665449178
    [
        638833882, 2136412568, 597143319, 2024118083, 526963908, 1681180171, 34365200, 1212065448,
        999657699, 62763571, 377848658, 1057379347, 1919391281, 173584178, 2011084033, 5669
    ],
    // montgomery R^2 mod p
    // 457845372202231092221045514406304715517609899600516288088351276206864288839367561156406646278891945147846188034105187428603489846554823930520200
    [
        271164040, 1165891739, 1379806029, 280729927, 415866605, 952876319, 1053763428, 834139733,
        654278754, 487382646, 638686566, 1333263944, 441449502, 1062024845, 1840416563, 4805
    ],
    // -p[0]^-1
...
fp31!(
    ...
    // 14371096341923360991777622009724443381605089814088122291496583894632133693821
    [
        1647710589, 393151430, 992158831, 1714616798, 1780437328, 1433211153, 1690175819,
        1658866359, 31
    ],
    // montgomery R^2 mod p
    // 56183755162440825278402343989826228114881341512867538765087816584313329438550
    [
        2118405974, 943949822, 1270792970, 83266189, 263283988, 230190960, 148909507, 460354326,
        124
    ],
    // -p[0]^-1
    // in sage: m = p.digits(2^31)[0]
...
mod lib {
    ...
    fn mont_mult2() {
        ...
        // a * R % fp_256::PRIME = 53194880143412583465331226137168779049052990239199584692423732563380439592004
    ...
    fn static_add_31_bit(){
        ...
        let expected = fp31_256::Fp256::new([1687077409,
 1547669063,
 1685320481,
 1036948901,
 4206667,
 ...
 91]);
        ...
        let a = fp31_256::Fp256::new([558607428,
 108819344,
 866477261,
 408251927,
 1279719733,
 ...
 117]
    ...
    #[test]
    fn mont_mult_31_bit() {
        let a = fp31_256::Fp256::new([1, 0, 4, 0, 16, 0, 64,0,0]);
        // a * R % fp31_256::PRIME = 53194880143412583465331226137168779049052990239199584692423732563380439592004
        let expected = fp31_256::Fp256::new([558607428,
 108819344,
 866477261,
 408251927,
 1279719733,
 ...
 117]);
    ...
    #[test]
    fn mont_mult3() {
        // a * a % fp_480::PRIME = 18140795192019734849437593733938181448350210532550492080276007701613892802183
        let a = fp_256::Fp256::new([1u64; fp_256::NUMLIMBS]);
        let expected = fp_256::Fp256::new([
            14975423715277739655,
            13454314221557599687,
            2400683194666791604,
            2889995408191712097,
        ]);
        assert_eq!((a.to_monty() * a.to_monty()).to_norm(), expected);
