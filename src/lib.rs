...
fp31!(
    ...
    8,      // Number of limbs when in 62 bit (ceil(bits/62))
    [
    ...
    // This is the reduction const, which is computed using 62 bit limb math since it's used only in multiplication.
    //2^(62*(2*8-1)) mod p
    ...
    // Montgomery One is R mod p, using 62 bit for the word size because it's only used in multiplication.
...
fp31!(
    ...
    // Montgomery One is R mod p, which is computed using 62 bit limb math since it's used only in multiplication.
    // montgomery R = 2^(W*N) where W = word size and N = limbs
    //            R = 2^(5*62) = 2^310
    // 34475546371888944175481977063149909728101006622996825201390467458658933952845
    [
        937972045, 1340669278, 2055201855, 1160985577, 603157176, 856919852, 381081696, 473685825,
        76
    ],
    // montgomery R^2 mod p, which is computed using 62 bit limb math since it's used only in multiplication.
    // 45066656446401961321162181312428777956457580069994368603299244339168090274348
    [
        728250924, 1903579402, 1188423838, 1856556933, 1539331604, 2079525121, 1817173117,
        1365863528, 99
    ],
    // -p[0]^-1
    // in sage: m = p.digits(2^31)[0]
...
mod lib {
    ...
    fn thirty_one_to_sixty_two() {
        ...
        let result = fp_256::thirty_one_to_sixty_two(thirty);
        assert_eq!(result, expected_result);
    }

    #[test]
    fn sixty_two_to_thirty_one() {
        let expected_result = [
            1001314762, 222542809, 1966841077, 1532144542, 1509311353, 1324885496, 689426205,
            1636449281, 61,
        ];
        ...
        let result = fp_256::sixty_two_to_thirty_one(sixty);
        assert_eq!(result, expected_result);
    }
    ...
    fn mont_mult2() {
        ...
        let expected = fp_256::Fp256::new([
            1319695786, 1928562400, 516909507, 329402382, 2120034460, 650495571, 1726608924,
            1552461525, 50,
        ]);
